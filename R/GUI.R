# The HybRIDS GUI Generator Reference Class for using HybRIDS with a graphical user interface.
  ...
  methods = list(
    ...
    AnalysisDisplay =
      function(sel) {
        ...
        l1 <- glabel(paste(t[5:7], sep="\n"), cont=f1)
        f2 <- gframe(paste(t[9]), cont=tempwin)
        l2 <- glabel(paste(t[c(11,12)], sep="\n"), cont=f2)
        f3 <- gframe(paste(t[14]), cont=tempwin)
        l3 <- glabel(paste(t[16:17], sep="\n"), cont=f3)
        f4 <- gframe(paste(t[19]), cont=tempwin)
        l4 <- glabel(paste(t[21:23], sep="\n"), cont=f4)
        f5 <- gframe(paste(t[25]), cont=tempwin)
        l5 <- glabel(paste(t[27:28], sep="\n"), cont=f5)
        l6 <- glabel(t[30], cont=tempwin)
    ...
    TestDateHandler =
      function(){
        ...
        DateBlockButton <- gbutton("Date Blocks", cont = DatingWindow, handler=function(h,...){
          ...
          gmessage("Finished Testing and Dating all Blocks", icon="info")
